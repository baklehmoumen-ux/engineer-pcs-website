...
export default function Storefront() {
    ...
    async function fetchData() {
      try {
        const [prodRes, slideRes] = await Promise.all([
          ...
          supabase.from('new_sliding_hero').select('*').eq('is_active', true).order('sort_order', { ascending: true })
        ]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans pb-24 md:pb-0 relative" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* 🌟 PREMIUM HEADER NAV WITH ICON-ONLY INSTAGRAM AND WHATSAPP */}
      <div className="bg-[#101423] text-gray-300 text-[11px] md:text-xs py-2 px-3 md:px-6 flex justify-between items-center z-50 relative border-b border-gray-800/50">
        ...
        <div className="flex gap-2 md:gap-4 items-center">
          ...
          {/* 🌟 AI SEARCH BUTTON */}
          <button onClick={() => setIsSearchOpen(true)} className="hover:text-indigo-400 text-white font-bold flex items-center gap-1.5 transition text-xs bg-gray-800 hover:bg-gray-700 px-2.5 md:px-3 py-1.5 rounded-full border border-gray-700 cursor-pointer shadow-sm group">
            <span className="group-hover:scale-110 transition-transform">🧠</span> <span className="hidden sm:inline">Search</span>
          </button>
          ...
          <a href="https://instagram.com/engineer_pcs" target="_blank" rel="noreferrer" aria-label="Instagram" title="Instagram" className="hover:text-pink-400 flex items-center justify-center p-1.5 bg-gray-800 hover:bg-gray-700 rounded-full border border-gray-700 transition group">
          ...
          {/* ICON-ONLY WHATSAPP BUTTON */}
          <a href="https://wa.me/963946508988" target="_blank" rel="noreferrer" aria-label="WhatsApp" title="WhatsApp" className="hover:text-green-400 flex items-center justify-center p-1.5 bg-gray-800 hover:bg-gray-700 rounded-full border border-gray-700 transition group">
            <svg className="w-3.5 h-3.5 fill-green-500 group-hover:scale-110 transition-transform" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
          </a>
        </div>
      </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-gray-50">
              <div>
                ...
                <div className="flex flex-col gap-1">
                  {categories.map((c) => (
                    <button key={c} onClick={() => { setActiveCategory(c); setIsMobileMenuOpen(false); document.getElementById('product-catalog')?.scrollIntoView({ behavior: 'smooth' }); }} className={`text-left text-sm font-bold px-3 py-2 rounded-lg transition ${activeCategory === c ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-200'}`}>
                      {t.categories[c] || c}
                    </button>
                  ))}
                </div>
                ...
                <div className="space-y-4 pb-20 md:pb-0">
                  ...
                  <div className="bg-gradient-to-br from-gray-900 via-[#1a233a] to-blue-900 text-white p-5 rounded-2xl shadow-lg border border-gray-800 relative overflow-hidden animate-fade-in-up">
                    ...
                    <p className="text-gray-300 text-xs leading-relaxed font-medium relative z-10">
                      {lang === 'ar' 
                        ? 'مرحباً بك في مساحتك المخصصة. يتم فحص كل قطعة تختارها تلقائياً لضمان التوافق التام وااحتياجات الطاقة. صمم حاسوبك بكل ثقة.' 
